...
__global__ void getMinimumDistanceCentroidIndex_kernel(float *existingCentroids, int newItem, float *dist);
...
void calculateThreshold(float **thresholdPtrPtr, float *thresholdP, int *newItems, int newItemsCount) {
	...
	***/
	int i, j, total_dist = 0, loopIndex, temp;
...
int getMinimumDistanceCentroidIndex_CPU(int newItem, float *existingCentroids, int existingCentroidsCount, float threshold) {
	/****
	...
	****/
	...
	int i, minimum, minimum_index;
	float *dist;
    float temp;
	if (existingCentroidsCount == 0)
		return MIN_DIST_CENTROID_NOT_FOUND;

	dist = (float *)malloc(existingCentroidsCount * sizeof(float));
	for (i = 0; i < existingCentroidsCount; i++)
	{
		temp = (existingCentroids[i] - newItem);
		dist[i] = temp * temp;
	}
...
int getMinimumDistanceCentroidIndex_GPU(int newItem, float *existingCentroids, int existingCentroidsCount, float threshold) {

	/**** Finding minnimum distance centroid index using GPU ****/

    int i, minimum, minimum_index; 
  
	float *dist, *dist_d;
  
    float *existingCentroids_d;
...
/* Allocating GPU memory */
    CUDA_CALL(cudaMalloc((void**)&existingCentroids_d, existingCentroidsCount * sizeof(float)));
    CUDA_CALL(cudaMalloc((void**)&dist_d, existingCentroidsCount * sizeof(float)));
/* Copies data from Host to device or device to Host - cudaMemcpy */
    CUDA_CALL(cudaMemcpy(existingCentroids_d, existingCentroids, existingCentroidsCount * sizeof(float), cudaMemcpyHostToDevice));
  //  printf("In GPU call\n");
  //  printf("\nIn CPU call of getMinimumDistanceCentroidIndex: ExistingCentroidCount:- %d\n", existingCentroidsCount);
    getMinimumDistanceCentroidIndex_kernel << <100, existingCentroidsCount >> >(existingCentroids_d, newItem, dist_d);
  //  printf("\nAfter the call of getMinimumDistanceCentroidIndex_GPU kernel call");
    CUDA_CALL(cudaMemcpy(dist, dist_d, existingCentroidsCount * sizeof(float), cudaMemcpyDeviceToHost));

      
    /*Free GPU memory */
    CUDA_CALL(cudaFree(existingCentroids_d));
...
/**** GPU function to get minimum distance centroid index****/
__global__ void getMinimumDistanceCentroidIndex_kernel(float *existingCentroids, int newItem, float *dist)
{
    int i = threadIdx.x;
    int temp;
...
/****Centroid calcualation function ****/
...
void addNewCluster(
	...
	float *existingCentroids, int *existingCentroidsCountPtr, int *clusterNumItems) { // H:- Added: int *clusterNumItems
...
void addToExistingCluster(int minDistClusterIdx, int newItem, int *existingGroups, int *existingGroupsCountPtr, int *clusterNumItems, float *existingCentroids) { // H:- Added: int *existingGroupCountPtr

	centroid_calculation(existingCentroids, minDistClusterIdx, clusterNumItems[minDistClusterIdx], newItem); // H:- Added 
	existingGroups[*existingGroupsCountPtr] = minDistClusterIdx; // H:- Added 
	clusterNumItems[minDistClusterIdx] += 1; // H:- Added 
	*existingGroupsCountPtr += 1;
	/****
	...
	Also, update the cluster_num_items array // H:- Added
	****/
}
...
int main(){
	...
	for (i = 0; i < newItemsCount; i++) {
        ...
        if (existingCentroidsCount > 65)
            minDistCentroidIndex = getMinimumDistanceCentroidIndex_GPU(
			                          newItem, existingCentroids, existingCentroidsCount, threshold);
        else
            minDistCentroidIndex = getMinimumDistanceCentroidIndex_CPU(
			                                      newItem, existingCentroids, existingCentroidsCount, threshold);
if (minDistCentroidIndex == MIN_DIST_CENTROID_NOT_FOUND) {
			addNewCluster(newItem, existingGroups, &existingGroupsCount, existingCentroids, &existingCentroidsCount, clusterNumItems); // H:- Added: &existingGroups, &existingGroupsCount, &clusterNumItems
		}
		else {
			addToExistingCluster(minDistCentroidIndex, newItem, existingGroups, &existingGroupsCount, clusterNumItems, existingCentroids); // H:- Added: &existingGroupsCount, &clusterNumItems
		}

	}

	print_results(existingGroups, existingGroupsCount, existingCentroids, existingCentroidsCount, threshold); // H:- Added //Harini
